...
* Synopsis
An /extremely/ minimal Emacs starter kit that uses no external packages by default, and only GNU-ELPA packages on an opt-in basis. Intended to be copied once and then modified as the user grows in knowledge and power.
...
- 2.0.0
  ...
  *Major release; new minimum supported Emacs version is 31.1.*
  ...
  Add MELPA to package archive list by default so Citar installs cleanly.
